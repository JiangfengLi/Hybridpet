...
- 结合中的舱处于 `sealing` 或 `generating` 时，玩家位于其中心水平 10 米内、
...
只有装备爱心辐射枪、游戏正常运行、命中舱已合拢且内部正在结合时才累计状态。
...
回调拿到只读快照，某个回调抛错不会阻断主循环。当前没有挂载会改变模型或基因的效果，
也不会因为命中重新提交、重试或加速付费 API 任务。
...
模型早于 90 秒完成也可就绪；超过 90 秒仍继续等待，合舱动画与爱心辐射枪交互继续运行。
...
- `build-standalone.mjs`：连同遗骸物理，合计内联 20 个模块，原六份音频保留。
